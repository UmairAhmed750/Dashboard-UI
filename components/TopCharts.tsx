...
export default function Charts() {
  ...
  const expenseOptions: any = {
    ...
    grid: { strokeDashArray: 5, borderColor: 'var(--chart-grid, #f1f5f9)' },
    dataLabels: { enabled: false },
  ...
  // 3. Product Sales Options (Donut)
  const productOptions: any = {
    chart: { type: 'donut', offsetY: -20 },
    colors: ['#0095FF', '#10B981', '#8B5CF6', '#FF5B7D'],
    stroke: { colors: ['var(--chart-stroke, #ffffff)'] }, // Dark mode mein donut borders handle karne ke liye
    plotOptions: {
      pie: {
  ...
  // Helper component legend ke liye (Text colors ko dark: classes di hain)
  ...
  return (
    // ApexCharts ke internal components ko dynamically range badalne ke liye humne main div par CSS variables inject kiye hain
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 max-w-400 mx-auto px-4 md:px-10 mt-6 [--chart-grid:#f1f5f9] [--chart-text:#1e293b] [--chart-stroke:#ffffff] dark:[--chart-grid:#334155] dark:[--chart-text:#f8fafc] dark:[--chart-stroke:#1e1e24]">
        ...
        <div className="flex flex-col xl:flex-row items-center justify-between mt-4 pt-6 border-t border-slate-50 dark:border-border gap-6">
          <div className="flex flex-col sm:flex-row gap-6 md:gap-10 w-full sm:w-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xl md:text-2xl font-bold text-slate-800 dark:text-foreground">
                  $63,489.50 <span className="text-[10px] bg-[#D1FAE5] dark:bg-emerald-500/20 text-[#10B981] px-2 py-1 rounded-full">+8%</span>
                </p>
                <p className="text-sm text-slate-400 dark:text-muted-foreground">Profit this year</p>
              </div>
            </div>

            {/* Stat 2 */}
            <div className="flex items-center gap-3 text-slate-400">
              <div className="bg-slate-50 dark:bg-secondary p-3 rounded-2xl border border-slate-100 dark:border-border shrink-0 text-slate-500 dark:text-muted-foreground">
                <DollarSign size={24} />
              </div>
              <div>
                <p className="text-xl md:text-2xl font-bold text-slate-800 dark:text-foreground">$38,496.00</p>
                <p className="text-sm text-slate-400 dark:text-muted-foreground">Expenses this year</p>
              </div>
            </div>
          </div>

          <button className="w-full xl:w-auto bg-[#0095FF] text-white px-8 py-3 rounded-xl font-bold text-sm hover:bg-blue-600 transition-all shadow-lg shadow-blue-100 dark:shadow-none hover:cursor-pointer">
            View Details
          </button>
        </div>
      </div>
      ...
      <div className="lg:col-span-1 bg-white dark:bg-card dark:border dark:border-border p-6 md:p-8 rounded-[2.5rem] shadow-sm flex flex-col h-auto lg:h-120">
        ...
        <div className="relative grow flex flex-col items-center justify-center -mt-10">
          ...
          <div className="absolute top-[65%] bg-[#D1FAE5] dark:bg-emerald-500/20 text-[#10B981] px-4 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1 shadow-sm uppercase tracking-wider">
            <Zap size={14} fill="currentColor" /> Best Seller
          </div>
        </div>

        {/* Legends Grid */}
        <div className="grid grid-cols-2 gap-4 border-t border-slate-50 dark:border-border pt-6">
